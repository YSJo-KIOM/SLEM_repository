...
def main():
	...
	if w1_path != None and w2_path != None and w3_path != None:
		...
		w1_data = pd.read_csv('new_weights/newPFC_geno_iso_weights_xv.txt',sep='\t',index_col=0)
		w2_data = pd.read_csv('new_weights/newPFC_iso_marker_weights_xv.txt',sep='\t',index_col=0)
		w3_data = pd.read_csv('new_weights/newPFC_marker_pheno_weights_xv_cell.txt',sep='\t',index_col=0)
		w_dist = 'xv'
	...
	if mode == 'save':
		...
		for i in range(10):
			...
			for j in range(1,11):
				num = 10*i + j
				#proc = Process(target=model_save,args=(X, Y, X, Y, W1, W2, B1, B2, b_size, epo, rate, True, linear, out_path+'rand'+str(num),(in_features, L1_features, L2_features),))
				proc = Process(target=model_save,args=(X, Y, X, Y, W1, W2, W3, B1, B2, B3, b_size, epo, rate, False, linear, w_dist, out_path+'_prior_'+str(num),feats,))
				procs.append(proc)
				proc.daemon = True
				proc.start()
			for proc in procs:
		...
		for i in range(10):
			...
			for j in range(1,11):
				num = 10*i + j
				#proc = Process(target=model_save,args=(X, Y, X, Y, W1, W2, B1, B2, b_size, epo, rate, True, linear, out_path+'rand'+str(num),(in_features, L1_features, L2_features),))
				proc = Process(target=model_save,args=(X, Y, X, Y, W1, W2, W3, B1, B2, B3, b_size, epo, rate, True, linear, w_dist, out_path+'_rand_'+str(num),feats,))
				procs.append(proc)
				proc.daemon = True
				proc.start()
			for proc in procs:
